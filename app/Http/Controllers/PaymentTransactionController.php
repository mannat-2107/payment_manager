    ...
    public function index(Request $request)
    {
        ...
        if ($request->search) {
            $query->where(function ($q) use ($request) {
                $q->whereHas('employee.user', function ($q2) use ($request) {
        ...
        if ($request->payroll_record_id) {
            ...
            if ($payroll) {
                if ($payroll->employee_id != $request->employee_id) {
                    return redirect()->route('transactions.create')->with('error', 'The selected payroll record does not belong to the selected employee.');
                }
