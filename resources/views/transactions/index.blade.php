    ...
    <div class="min-h-screen bg-gray-50">
        ...
        @if($approvedCount > 0)
            <form method="POST" action="{{ route('transactions.bulk-pay') }}"
                ...
                @csrf
                <button type="submit"
                    class="bg-green-600 text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-green-700 transition flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    Bulk Pay ({{ $approvedCount }})
                </button>
            </form>
        @endif
                ...
                <div class="stat-card bg-white rounded-2xl border border-gray-100 p-5 ani-3">
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <div class="w-9 h-9 bg-yellow-50 rounded-xl flex items-center justify-center">
                            <svg class="w-5 h-5 text-yellow-600 pulse" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <tr class="txn-row">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-9 h-9 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-xs font-semibold flex-shrink-0">
                                            {{ strtoupper(substr($txn->employee->user->name, 0, 2)) }}
                                        </div>
                                        <div>
                                            <p class="text-sm font-semibold text-gray-800">{{ $txn->employee->user->name }}
                                            </p>
                                            <p class="text-xs text-gray-400 font-mono">{{ $txn->employee->employee_code }}
                                            </p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span
                                        class="text-xs font-mono text-gray-500 bg-gray-50 px-2 py-1 rounded-lg">{{ $txn->transaction_reference }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <span
                                ...
                                <td class="px-6 py-4">
                                    <span
                                        class="text-sm text-gray-500 capitalize">{{ str_replace('_', ' ', $txn->payment_method) }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    @if($txn->status === 'success')
                                        <span
                                            class="inline-flex items-center gap-1.5 bg-green-100 text-green-700 text-xs px-3 py-1.5 rounded-full font-medium">
                                            <span class="w-1.5 h-1.5 bg-green-500 rounded-full"></span>Success
                                        </span>
                                    @elseif($txn->status === 'processing')
                                        <span
                                            class="inline-flex items-center gap-1.5 bg-yellow-100 text-yellow-700 text-xs px-3 py-1.5 rounded-full font-medium">
                                            <span class="w-1.5 h-1.5 bg-yellow-500 rounded-full pulse"></span>Processing
                                        </span>
                                    @elseif($txn->status === 'initiated')
                                        <span
                                            class="inline-flex items-center gap-1.5 bg-blue-100 text-blue-700 text-xs px-3 py-1.5 rounded-full font-medium">
                                            <span class="w-1.5 h-1.5 bg-blue-500 rounded-full"></span>Initiated
                                        </span>
                                    @elseif($txn->status === 'failed')
                                        <span
                                            class="inline-flex items-center gap-1.5 bg-red-100 text-red-700 text-xs px-3 py-1.5 rounded-full font-medium">
                                            <span class="w-1.5 h-1.5 bg-red-500 rounded-full"></span>Failed
                                        </span>
                                    @else
                                        <span
                                            class="inline-flex items-center gap-1.5 bg-gray-100 text-gray-700 text-xs px-3 py-1.5 rounded-full font-medium">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <a href="{{ route('transactions.receipt', $txn) }}"
                                            class="text-xs bg-purple-50 border border-purple-200 text-purple-600 px-2 py-1 rounded-lg hover:bg-purple-100 transition font-medium">
                                            Receipt
                                        </a>
                                        @if($txn->status !== 'success')
                                            <form method="POST" action="{{ route('transactions.update', $txn) }}">
                                                ...
                                                <select name="status" onchange="this.form.submit()"
                                                    ...
                                                    <option value="initiated" {{ $txn->status == 'initiated' ? 'selected' : '' }}>
                                                        Initiated</option>
                                                    <option value="processing" {{ $txn->status == 'processing' ? 'selected' : '' }}>Processing</option>
                                                    <option value="success" {{ $txn->status == 'success' ? 'selected' : '' }}>
                                                        Success</option>
                                                    <option value="failed" {{ $txn->status == 'failed' ? 'selected' : '' }}>Failed
                                                    </option>
                                                    <option value="reversed" {{ $txn->status == 'reversed' ? 'selected' : '' }}>
                                                        Reversed</option>
                                                </select>
                                            </form>
                                        @endif
                            ...
                            <tr>
                                <td colspan="7" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center gap-3">
                                        ...
                                        <a href="{{ route('transactions.create') }}"
                                            class="text-blue-600 text-sm hover:underline">Create your first transaction
                                            →</a>
                                    </div>
                                </td>
                            </tr>
